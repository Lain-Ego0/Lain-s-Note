...
$$
其中，$r_t(\theta)$ 是新旧策略的概率比，$\epsilon$ 是超参数。当新策略与旧策略差异过大时，目标函数会被截断。此函数强制限制策略更新幅度，形成类似 TRPO 的信任区域，但无需计算复杂的共轭梯度或海森矩阵，极大降低计算成本并保证性能单调改进。

另外，在机器人运动控制、自动驾驶等物理仿真场景中，动作通常是连续数值。PPO 的 Actor 网络可直接输出高斯分布的均值 ($\mu$) 和标准差 ($\sigma$)，从而直接采样连续动作，更契合物理控制任务。相比于 DDPG 或 AC 算法，PPO 对学习率、初始化权重等超参数不敏感，默认参数即可在多种任务中获得良好表现 。同时PPO 仅需一阶梯度优化，易于在 PyTorch 中实现，且便于扩展到大规模并行环境。
