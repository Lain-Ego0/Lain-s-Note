...
如果在您的电脑上还没有安装 Node.js，您必须先安装它。

1. 访问 [Node.js 官网](https://nodejs.org/)。

2. 下载 **LTS (长期支持版)**，因为这个版本最稳定。
...
4. **验证安装**：打开您的终端（Windows 下是 CMD 或 PowerShell，Mac/Linux 下是 Terminal），输入以下命令并回车：
   ...
   如果您看到了版本号（例如 `v20.x.x`），说明安装成功。

------

### 第二步：获取并解压代码

1. **下载代码**：您应该已经从 AI Studio 下载了一个压缩包（通常是 `.zip` 文件）。
...
- **发生了什么？** 计算机正在读取项目中的 `package.json` 文件，并自动下载项目运行所需的所有第三方库（依赖包）。这可能需要几分钟，取决于您的网络速度。

------

### 第四步：配置 API 密钥 (最关键的一步)

项目需要连接 Google 的服务器，所以需要您的“通行证”（API Key）。
...
2. **创建环境变量文件**：
   ...
   - 回到您的项目文件夹。
   - 查看是否有 `.env.local` 文件。如果没有，您需要创建一个。
   - *如果是 Windows*：右键新建文本文档，命名为 `.env.local`（**注意：** 确保没有 `.txt` 后缀。如果系统隐藏了后缀名，可能需要先开启“显示文件扩展名”）。
   - 或者，如果项目里有一个叫 `.env.example` 的文件，您可以将它复制一份并重命名为 `.env.local`。

3. **填入密钥**：

   - 用记事本或代码编辑器（如 VS Code）打开 `.env.local`。

   - 输入以下内容（将 `您的_API_KEY_粘贴在这里` 替换为您刚才复制的 `AIza...` 那个字符串）：
